...
impl<T> TileConstraints<T> {
    ...
    pub fn update_cell<const C: usize>(
        &self,
        (cell_tiles, cell_position): (PossibleIndices, Position),
        neighbours_iter: impl IntoIterator<Item = (PossibleIndices, Position)>,
        all_weights: &[f32],
        tiles: &[T],
    ) -> PossibleIndices {
        ...
        let mut new_indices = PossibleIndices::default();
...
impl<T> Constraint<T> for PossibleNeighbours {
    fn valid(
        ...
        let valid = neighbours.iter().all(|(nts, _)| {
            nts.iter().any(|nt| {
